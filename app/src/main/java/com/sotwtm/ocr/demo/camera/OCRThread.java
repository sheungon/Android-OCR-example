...
public class OCRThread extends Thread {
    ...
    private void updateTextRegions() {
        final TextRegionsListener listener = this.regionsListener;
        if (listener != null) {
            final List<Rect> regions = textRecognitionHelper.getTextRegions();
            handler.post(new Runnable() {
                @Override
                public void run() {
                    listener.onTextRegionsRecognized(regions);
                }
            });
    ...
    private synchronized void updateOCRText() {
        final TextRecognitionListener listener = this.textRecognitionListener;
        if (listener != null) {
            ...
            OCRThread.this.textRecognitionListener = null;
        }
    }
