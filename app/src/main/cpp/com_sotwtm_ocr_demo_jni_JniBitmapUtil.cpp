#include "com_sotwtm_ocr_demo_jni_JniBitmapUtil.h"
...
JNIEXPORT void JNICALL Java_com_sotwtm_ocr_demo_jni_JniBitmapUtil_decode
(JNIEnv *env, jclass javaClass, jobject target, jbyteArray source) {
    AndroidBitmapInfo lBitmapInfo;
    if (AndroidBitmap_getInfo(env, target, &lBitmapInfo) < 0) {
        return;
    }
    ...
    if (AndroidBitmap_lockPixels(env, target, (void **) &lBitmapContent) < 0) {
        return;
    }
    jbyte *lSource = (jbyte *) env->GetPrimitiveArrayCritical(source, 0);
    ...
    env->ReleasePrimitiveArrayCritical(source, lSource, 0);
